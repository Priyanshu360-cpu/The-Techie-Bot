...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
        const { MessageActionRow, MessageEmbed,  MessageAttachment, MessageButton,  MessageSelectMenu } = require('discord.js');
...
async function catcher(){
    let o = await message.channel.send({embeds:[embed],components:[column,row]})
function edito(a,i){
...
i.deferUpdate()
    if(i.customId === 'avatar'){
        ...
        if(i.values[0] == "webp"){
            ...
            a.setTitle(embed.title)
            a.setDescription(embed.description)
